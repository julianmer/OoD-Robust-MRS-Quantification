...
#   draw parameters   #
#*********************#
def simulateParam(basis, batch, params=normParams, concs=normConcs, sigModel=None):
    """
    Function to simulate SVS parameters.

    @param basis -- The basis set of metabolites to simulate (as Basis object).
    ...
    @param sigModel -- The signal model to use, if not given the default model will be used.
    ...
    elif 'noise_db' in params:
        noise_db = dist(params['noise_db'][0], params['noise_db'][1], batch)
        if sigModel is None:
            sigModel = VoigtModel(basis=basis.basis, first=0, last=basis.n, t=basis.t, f=basis.f)
        spec = sigModel.forward(torch.from_numpy(np.swapaxes(theta, 0, 1)), sumOut=True, baselineOut=False)   # ATTENTION: baseline now in signal power
        signal_power = torch.mean(torch.abs(spec) ** 2, dim=1).numpy()
        sigma = np.sqrt(signal_power / (10 ** (noise_db / 10)))
        noise = np.random.normal(0, sigma[:, np.newaxis], (batch, basis.fids.shape[0])) + \
                1j * np.random.normal(0, sigma[:, np.newaxis], (batch, basis.fids.shape[0]))
    else:
