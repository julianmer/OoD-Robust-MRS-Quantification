...
from utils.auxiliary import general_minimize, scaleConcentrations
...
class Framework():
    def __init__(self, path2basis, basisFmt='', specType='all', dataType='none', ppmlim=None):
        ...
        elif dataType == 'aumc2_ms': self.ps, self.concs = aumcParams2, aumcConcsMS
        ...
        elif dataType == 'fip_rw_p': self.ps, self.concs = fipParamsRWP, normConcs
        elif dataType == 'fip_rw_p2': self.ps, self.concs = fipParamsRWP2, normConcs
    ...
    #**************************#
    def scaleConcs(self, x, y, x_ref, TE, TR, frac=None):
        x = x[:, 0] + 1j * x[:, 1]
        fids = np.fft.ifft(x, axis=-1)  # to time domain

        concs = []
        for i in range(x.shape[0]):
            conc = scaleConcentrations(concs=y[i], basis=self.basisFSL,
                                       fid=fids[i], ref=x_ref[i], TE=TE, TR=TR,
                                       frac=frac[i] if frac is not None else None)
            concs.append(conc)
        concs = torch.tensor(np.array(concs), device=x.device, dtype=x.dtype)
        return concs
    ...
    #   optimal referencing   #
    #*************************#
    def optimalReference(self, y, y_hat, type='scipy_l1', exMM=False):
        # y_hat = self.scaleEstimate(y, y_hat)   # roughly scale the estimate

        if exMM:   # exclude macromolecules
    ...
    #****************************#
    def concsLoss(self, t, t_hat, type='smae', exMM=False):
        ...
        if type == 'mae':    # mean absolute error
            ...
            mae = self.concsLoss(t, t_hat, type='mae')
            return torch.clamp(100 * mae / torch.clamp(t, min=torch.finfo(mae.dtype).eps), max=999)

        elif type == 'smape':   # scaled mean absolute percentage error
            smae = self.concsLoss(t, t_hat, type='smae')
            return torch.clamp(100 * smae / torch.clamp(t, min=torch.finfo(smae.dtype).eps), max=999)
