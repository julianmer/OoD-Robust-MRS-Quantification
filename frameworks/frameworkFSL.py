...
class FrameworkFSL():
    ...
    def fsl_minimize(self, x, x_ref=None, frac=None, x0=None):
        ...
        if self.multiprocessing:
            tasks = [(fid, self.basisFSL, self.method, x_ref, frac, x0, i)
    ...
    def fsl_forward(self, fid, basis, method, ref=None, frac=None, x0=None, idx=0):
        ...
        if ref is not None:   # water scaling
            ...
            try:
                ...
                concs = np.array([concs[opt.metabs.index(m)] for m in self.basisFSL.names])
                uncs = np.array([uncs[opt.metabs.index(m)] for m in self.basisFSL.names])

                if self.include_params:   # also return the signal parameters...
                                          # !!! this will affect the fit
                    concs = np.concatenate((concs, opt.params[len(self.basisFSL.names):]), axis=0)
                    uncs = np.concatenate((uncs, opt.crlb[len(self.basisFSL.names):]), axis=0)
            except:
        ...
        if self.unc == 'perc':   # uncertainty estimation
            with np.errstate(divide='ignore', invalid='ignore'):
                perc_SD = np.sqrt(opt.crlb[:len(concs)]) / concs * 100
...
if __name__ == '__main__':
    ...
    fsl = FrameworkFSL(config['path2basis'], method=config['method'],
    ...
    data = np.array([mrs_io.read_FID(config['path2data'] + file).mrs().FID
                     for file in files])

    if 'path2water' in config:
        files = os.listdir(config['path2water'])[:config['test_size']]
        water = np.array([mrs_io.read_FID(config['path2water'] + file).mrs().FID
                          for file in files])
    else:
        water = None
