...
#**************************#
def scaleConcentrations(concs, basis, fid, ref, TE, TR, frac=None, scaling='molarity',
                        model='voigt', method='custom', metab_groups=None, baseline_order=2,
                        ppmlim=(0.5, 4.2), nucleus='1H'):
    """
    Scales the concentrations of the metabolites

    @param concs -- The concentrations of the metabolites.
    @param basis -- The basis set.
    @param fid -- The FID of the spectrum.
    @param ref -- The reference concentration.
    @param TE -- The echo time (TE) in milliseconds.
    @param TR -- The repetition time (TR) in milliseconds.
    @param frac -- The fractions of the metabolites (default: None).
    @param scaling -- The scaling method (default: 'molarity').
    @param model -- The model used for fitting (default: 'voigt').
    @param metab_groups -- The metabolite groups to consider (default: None).
    @param baseline_order -- The order of the baseline polynomial (default: 2).
    @param nucleus -- The nucleus of interest (default: '1H').

    @returns -- The scaled concentrations.
    """
    mrs = MRS(FID=fid, basis=basis, H2O=ref, cf=basis.cf, bw=basis.original_bw, nucleus=nucleus)
    if metab_groups is None: metab_groups = [0] * len(mrs.names)
    B = prepare_baseline_regressor(mrs, baseline_order, ppmlim)
    res = FitRes(mrs=mrs, results=concs, model=model, method=method, metab_groups=metab_groups, 
                 baseline_order=baseline_order, B=B, ppmlim=ppmlim)

    q_info = quantify.QuantificationInfo(TE, TR, mrs.names, mrs.centralFrequency / 1E6)

    if frac is not None: q_info.set_fractions(frac[idx])

    res.calculateConcScaling(mrs, quant_info=q_info)
    concs = res.getConc(scaling=scaling)
    return concs
