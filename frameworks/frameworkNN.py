...
class FrameworkNN(Framework, pl.LightningModule):
    def __init__(self, path2basis, basisFmt='', specType='synth', dataType='none', data_dir=None,
                 ppmlim=None, norm=True, forwardNorm=True, val_size=1000, batch=16, lr=1e-3,
                 reg_l1=0.0, reg_l2=0.0, loss='mse', arch='mlp', **kwargs):
    ...
    def loss(self, x, y, t, theta, type='mae_conc'):
        ...
        elif type.lower() == 'mse_specs_scale':
            spec = self.sigModel.forward(theta)
            spec = processSpectra(spec, self.basis)

            err_real = torch.nn.MSELoss(reduction='none')(spec[:, 0, self.first:self.last],
                                                     x[:, 0, self.first:self.last])
            err_imag = torch.nn.MSELoss(reduction='none')(spec[:, 1, self.first:self.last],
                                                      x[:, 1, self.first:self.last])

            err_real /= torch.abs(x[:, 0, self.first:self.last]).mean(dim=-1).unsqueeze(-1)
            err_imag /= torch.abs(x[:, 1, self.first:self.last]).mean(dim=-1).unsqueeze(-1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.norm and not self.forwardNorm:
            norm = torch.sqrt(torch.sum(x[:, 0, self.first:self.last]**2 +
                                        x[:, 1, self.first:self.last]**2, dim=1, keepdim=True))
            norm = torch.clamp(norm, min=1e-8)  # avoid division
            x = x / norm.unsqueeze(-1)  # normalize the spectra
            t[:, :self.basisObj.n_metabs] /= norm
            t[:, -6:] /= norm  # scale the baseline (attention: assumes fix 2nd order baseline)

        loss = self.loss(x, y, t, t_hat, type=self.lossType)
        self.log("train_loss", loss.mean().item(), prog_bar=True)

        thetaLoss = self.loss(x, y, t, t_hat, type='mae_conc')
    ...
    def train_dataloader(self):
        while True:  # ad-hoc simulation
            batch = self.dataSynth.get_batch(self.batchSize)
            yield batch
    ...
    def val_dataloader(self):
        ...
        for _ in range(self.valSize):
            batch = self.dataSynth.get_batch(1)
            x, y, t = batch
