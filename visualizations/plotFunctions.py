...
#   plot spec and fit   #
#***********************#
def plot_spec_and_fit(spec, fit, true=None, ppmAxis=None, ppmLim=None, name='', save_path=None, real=True,
                      specOnly=False):
    """
        Plots the given spectrum and its fit in ppm scale.
        @param spec -- The spectrum to plot.
        @param fit -- The fit to plot.
        @param true -- The true spectrum to plot (default None).
        @param ppmAxis -- The ppm axis to plot (default None).
        @param ppmLim -- The ppm limits to plot (default None).
        @param name -- Title of the plot (default '').
        @param save_path -- The path to save the plot (default None).
        @param real -- If True, the real part is visualized, else the imag part (default True).
        @param specOnly -- If True, only the spectrum is plotted (default False).
    """
    if ppmAxis is None: ppmAxis = np.linspace(0.5, 4.2, spec.shape[0])

    if ppmLim is not None:
        beg = (np.abs(ppmAxis - ppmLim[0])).argmin()
        end = (np.abs(ppmAxis - ppmLim[1])).argmin()
        ppmAxis = ppmAxis[beg:end]
        spec = spec[beg:end]
        fit = fit[beg:end]
        if true is not None: true = true[beg:end]
    ...
    plt.plot(ppmAxis, spec, 'k', label='Data', linewidth=1)
    plt.plot(ppmAxis, fit, 'r', label='Fit', alpha=0.6, linewidth=2)
    plt.plot(ppmAxis, spec - fit + 1.1 * np.max(spec),
             'dimgrey', label='Residual', alpha=0.8, linewidth=1)
    if true is not None:
        true = np.real(true) if real else np.imag(true)
        plt.plot(ppmAxis, true, 'g', label='True Spectrum', alpha=0.8, linewidth=1, linestyle='--')
    ...
    if save_path is not None:
        ...
        plt.savefig(save_path + f'{name}.png', dpi=300, bbox_inches='tight', transparent=True)


#*****************#
#   plot dataset  #
#*****************#
def plot_dataset(specs, ppmAxis=None, ppmLim=None, name='', save_path=None, real=True):
    """
        Plots the given mean spectrum and all other spectra in ppm scale.
        @param specs -- The spectra to plot.
        @param ppmAxis -- The ppm axis to plot (default None).
        @param name -- Title of the plot (default '').
        @param save_path -- The path to save the plot (default None).
        @param real -- If True, the real part is visualized, else the imag part (default True).
    ...
    if real: specs = np.real(specs)
    else: specs = np.imag(specs)

    plt.figure()
    for i in range(specs.shape[0]):
        plt.plot(ppmAxis, specs[i], 'dimgrey', alpha=0.1, linewidth=0.3)
    plt.plot(ppmAxis, np.mean(specs, axis=0), 'k', label='Mean Spectrum', linewidth=2)
