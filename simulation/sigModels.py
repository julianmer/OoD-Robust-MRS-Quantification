...
class VoigtModel(SigModel):
    ...
    def compute_init_params(self, data, first=None, last=None):
        ...
        Parameters:
        - data (torch.Tensor): The measured data with shape (batch, n_freq).
        - first (int, optional): The first frequency index. Defaults to instance attribute.
        - last (int, optional): The last frequency index. Defaults to instance attribute.

        Returns:
        - tuple: Estimated parameters cat(con_init, gamma, sigma, eps, phi0, phi1, b_params) for each batch.
        """
        if first is None: first = self.first
        if last is None: last = self.last
    ...
    def err_func(self, theta, data, first=None, last=None, sumOut=True):
        """
    ...
    #   gradient model   #
    #********************#
    def gradient(self, theta, data, first=None, last=None, phase1=True, sumOut=True):
        """
        ...
        @param last -- The last frequency index.
        @param phase1 -- Whether to include the first-order phase (default: True).
        @param sumOut -- Whether to sum over the frequency range [first:last] (default: True).
        ...
        if sumOut:
            ...
            return grad_total
        else:   # no summing over frequency range
            grad_con = -2 * torch.real(E.unsqueeze(-1) * torch.conj(dfd_dcon[:, first:last, :]))  # (B, freq_range, n)
            grad_phi0 = -2 * torch.real(E * torch.conj(dfd_dphi0[:, first:last])).unsqueeze(-1)  # (B, freq_range, 1)
            grad_phi1 = -2 * torch.real(E * torch.conj(dfd_dphi1[:, first:last])).unsqueeze(-1)  # (B, freq_range, 1)
            grad_gamma = -2 * torch.real(E * torch.conj(dfd_dgamma[:, first:last])).unsqueeze(-1)  # (B, freq_range, 1)
            grad_sigma = -2 * torch.real(E * torch.conj(dfd_dsigma[:, first:last])).unsqueeze(-1)  # (B, freq_range, 1)
            grad_eps = -2 * torch.real(E * torch.conj(dfd_deps[:, first:last])).unsqueeze(-1)  # (B, freq_range, 1)
            if dfd_db is not None:
                grad_b = -2 * torch.real(E.unsqueeze(-1) * torch.conj(dfd_db[:, first:last, :]))
            else:
                grad_b = None
            grad_list = [grad_con, grad_gamma, grad_sigma, grad_eps, grad_phi0, grad_phi1]
            if grad_b is not None:
